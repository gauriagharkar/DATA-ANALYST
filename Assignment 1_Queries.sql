...
# Que. 02 Create Table From ComsolidatedTables.sql

create table assignment.countries(name varchar(255) not null,population int  not null
,capital varchar(255) not null);
create table assignment.movies (id integer, title varchar(50), category varchar(25));
create table assignment.rentals(memid integer, first_name varchar(25), last_name varchar(25), movieid integer);
...
create table assignment.employee (empid integer, fname varchar(30), lname varchar(30), deptno tinyint, salary decimal(10,2));
CREATE TABLE assignment.`station` (
...
CREATE TABLE assignment.`productlines` (
...
CREATE TABLE assignment.`orderdetails` (
  `orderNumber` int(11) NOT NULL,
  `productCode` varchar(15) NOT NULL,
...
CREATE TABLE assignment.`payments` (
  `customerNumber` int(11) NOT NULL,
...
# Que.03 : Countries Table

/*
create database countries;
use countries;
create table countries(name varchar(255) not null,population int  not null
,capital varchar(255) not null);
select *from countries;
*/

/* Que. 3 a) */
insert into assignment.countries values('China',1382,'Beijing'),('India',1326,'Delhi'),('United States',324,'Washington D.C.'),
...
select*from assignment.countries;

/* Que. 3 b) */
/* values added*/

insert into assignment.countries values('Spain',330,'Madrid'),('Ukraine',292,'Kyiv'),('North Karea',287,'Pyongyang');
...
update  assignment.countries set capital='New Delhi' where name='India' and population=1326;
select*from assignment.countries;

/* Que.4 */

alter table assignment.countries rename to assignment.big_countries; 

/* Que 8 a) */

alter table assignment.emp add deptno int;
/* alter table assignment.emp add deptno int after genter --> deptno will after gender column in enp table */ 
desc assignment.emp;

drop table assignment.emp;
...
update assignment.emp set deptno=20 where emp_id%2=0;
select *from assignment.emp;

update assignment.emp set deptno=30 where emp_id%3=0;
select *from assignment.emp;

update assignment.emp set deptno=40 where emp_id%4=0;
select *from assignment.emp;


update assignment.emp set deptno=50 where emp_id%5=0;
select *from assignment.emp;

update assignment.emp set deptno=10 where deptno is NULL;
select *from assignment.emp;

create unique index emp_id_index 
on assignment.emp(emp_id);

select*from assignment.emp;

create view assignment.emp_sal as 
select emp_id, first_name, last_name, salary from assignment.emp order by salary desc;

select*from assignment.emp_sal;


/* Que.05 */
use assignment;
create table assignment.product1(
...
describe assignment.product1;

create table assignment.supplier1(
    supplier_id int,
...
create table assignment.stocks1
(
    id int, 
    product_id int,
    balance_stock int,
...
alter table supplier1 modify supplier_name varchar(255) not null unique;
desc supplier1;

/* Que.06 */ 

truncate table assignment.product1;
insert into assignment.product1(product_id,product_name,supplier_id)
values (11,'Car',394),(12,'Truck',394),(21,'Bike',194),(22,'Cycle',194);

insert into assignment.supplier1(supplier_id,supplier_name,location)
values(394,'Yash','Delhi'),(523,'Nikhil','Mumbai'),(194,'Pravin','Pune');

insert into assignment.stocks1(id,product_id) values (1,21),(2,21),(3,11),(4,11);

select*from assignment.product1;
select*from assignment.supplier1;
select*from assignment.stocks1;
